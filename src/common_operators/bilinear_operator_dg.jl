...
function build_assembler!(A, O::BilinearOperatorDG{Tv}, FE_test, FE_ansatz, FE_args::Array{<:FEVectorBlock, 1}; time = 0.0, kwargs...) where {Tv}
    ...
    return if (O.FES_test != FES_test) || (O.FES_args != FES_args) || (O.FES_ansatz != FES_ansatz)
        ...
        for EG in EGs
            ...
            push!(O.BE_test_vals, [[O.BE_test[end][k][j[1], j[2]].cvals for j in CartesianIndices(O.BE_test[end][k])] for k in 1:ntest])
            push!(O.BE_ansatz_vals, [[O.BE_ansatz[end][k][j[1], j[2]].cvals for j in CartesianIndices(O.BE_ansatz[end][k])] for k in 1:nansatz])
        ...
        op_lengths_test = [size(O.BE_test[1][j][1, 1].cvals, 1) for j in 1:ntest]
        op_lengths_ansatz = [size(O.BE_ansatz[1][j][1, 1].cvals, 1) for j in 1:nansatz]
        ...
        FEATs_test = [ExtendableFEMBase.EffAT4AssemblyType(get_AT(FES_test[j]), ON_CELLS) for j in 1:ntest]
        FEATs_ansatz = [ExtendableFEMBase.EffAT4AssemblyType(get_AT(FES_ansatz[j]), ON_CELLS) for j in 1:nansatz]
...
function build_assembler!(A, O::BilinearOperatorDG{Tv}, FE_test, FE_ansatz; time = 0.0, kwargs...) where {Tv}
    ...
    return if (O.FES_test != FES_test) || (O.FES_ansatz != FES_ansatz)
        ...
        function assembler(A, b; kwargs...)
            return if O.parameters[:store] && size(A) == size(O.storage)
                ...
                time = @elapsed begin
                    ...
                    elseif O.parameters[:parallel_groups]
                        Threads.@threads for j in 1:length(EGs)
                            fill!(Aj[j].cscmatrix.nzval, 0)
                            assembly_loop(Aj[j], itemassemblygroups[j], EGs[j], O.QF[j], O.BE_test[j], O.BE_ansatz[j], O.BE_test_vals[j], O.BE_ansatz_vals[j], O.L2G[j], O.QP_infos[j]; kwargs...)
                            flush!(Aj[j])
                        end
                        for j in 1:length(EGs)
