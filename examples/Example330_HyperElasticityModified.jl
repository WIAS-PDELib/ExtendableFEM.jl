...
function nonlinkernel_DW!(κ)
    ...
    make_W(qpinfo) = (out, in) -> W!(κ)(out, in, qpinfo)
...
## derivative of energy functional (by ForwardDiff) for κ-derivative
function nonlinkernel_DW2!(κ)
    ## A dictionary with cached data for each input type
    AD_backend = AutoForwardDiff()
    jac_prep = nothing
    T = eltype(κ)
    ## first initialization of cached data when type of input = F is known
    scalar_dummy = zeros(T, 1)
    ...
    return function closure(result, input, qpinfo)
        W = make_W(qpinfo)
        if jac_prep === nothing
            jac_prep = prepare_jacobian(W, scalar_dummy, AD_backend, input)
        end
        jacobian!(W, scalar_dummy, result, jac_prep, AD_backend, input)
        return nothing
    end
end


function optim_functions(sol, B)
    ...
    function F(κ)
        ...
        res_vector.entries[fixeddofs] .= 0
        return dot(res_vector.entries, res_vector.entries)
    end
...
function main(;
        maxvolume = 0.1,  # parameter for grid generator
        E = 10,             # Young modulus
        ν = 0.3,            # Poisson ratio
    ...
    assign_unknown!(PD, u)
    assign_operator!(PD, NonlinearOperator(nonlinkernel_DW!([μ, λ]), [grad(u)]; sparse_jacobians = false))
